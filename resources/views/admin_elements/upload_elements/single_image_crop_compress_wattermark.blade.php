...
if ($(".product_master_id").val().length<=0) {
    alert("Please select crop before uploading image");
    return false;
}
var this_obj=$(this);
...
formData.append("product_master_id", $(".product_master_id").val());
  ...
  $.ajax({
    ...
    url: "{{ route('admin.single_image_crop_compress_wattermark') }}",
